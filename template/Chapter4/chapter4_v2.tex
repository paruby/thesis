...
It follows that $z_i$ is an invertible function of $h_j(x_1)$, and so $h(x_1)$ recovers $s$ up to permutations and coordinate-wise invertible transformations.
...
\begin{corollary}
	...
	\end{equation*}
	results in ${h}$ inverting ${f}_2$ in the sense that the $h_{i}({X}_2)$ recover the independent components of ${g}({Z}, {N})$ up  to component-wise invertible transformations.
\end{corollary}
...
\begin{figure}[t!]
	...
	\caption{Setting with $M$ corrupted views of the sources.}
...
Since each view provides additional information about $Z$, the question naturally arises: in the limit as $M \to \infty$, is it possible to reconstruct $\bm{s}$ exactly?
...
Kolmogorov's strong law says that the average of a sequence of independent random variables converges to the expectation of the average, provided that the variances of the random variables do not grow too quickly. 
...
This section discusses in further detail the Sufficiently Distinct Views assumption and the necessity for source-level noise for at least one of the views.
...
Suppose that conditional distribution is Gaussian, so that
...
The reasons for this are explained briefly in this section.\footnote{\todo{Rewrite if density ratio estimation is covered in the literature review}}
...
Recall that in the setting we consider, the function $r(x_1, x_2)$ is trained to classify between the two cases that $(x_1, x_2)$ is drawn from the joint distribution $p(x_1, x_2)$ (\emph{class $0$}) or the product of marginals $p(x_1) p(x_2)$ (\emph{class $1$}).
