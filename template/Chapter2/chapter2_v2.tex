...
ICA will be discussed in more detail in Chapter \ref{chapter:ica}, in which identifiability results are presented.
...
A Latent Variable Model (LVM) over a space $\mathcal{X}$ consists of a distribution $P_Z$ over a low dimensional space $\mathcal{Z}$ together with conditional distributions $P_{X|Z}$. 
...
If the generator is stochastic, it can still be represented simply as a neural network provided that the conditional distributions are sufficiently structured. 
...
A metric is additionally symmetric and must obey the triangle inequality.
...
\subsection{Integral Probability Metrics (IPMs)}\label{subsec:intro-ipm}
...
\item The Maximum Mean Discrepancy (MMD) where $\mathcal{H}$ is the set of functions in a reproducing kernel Hilbert space with norm at most $1$ induced by some kernel $k$ \citep{gretton2012kernel}.
\end{itemize}
...
To see that this $f$-divergences are indeed divergences, consider first positivity, which follows from convexity of $f$ and Jensen's inequality:
%
\begin{align}
D_f(P, Q) &= \int f\left(\frac{p(x)}{q(x)}\right) q(x) dx \\ 
...
It is also strict for any $f$ that does not have constant gradient in any neighbourhood of $1$, since the $f_0$ associated to any such $f$ is strictly positive on $\mathbb{R}_+\setminus \{1\}$.
...
Suppose that samples from $P$ are labelled as class $1$, and samples from $Q$ are labelled as class $0$. 
This can be implemented as logistic regression, in which case a function $c:\mathcal{X}\to [0,1]$ is introduced and trained to minimise the objective 
%
\begin{align*}
...
\end{align*}
%
For any particular $x$, the integrand is minimised by $c^*(x) = \frac{p(x)}{q(x) + p(x)}$ \citep{goodfellow2014generative}, and so the optimal classifier assigns to $x$ the posterior probability that it was drawn from $P$.
...
In fact, it can be similarly shown that any $f$-divergence between two distributions can be estimated by maximisation of a lower bound corresponding to a classification problem, and that doing so results in a function of the density ratio being estimated.
...
One example of an IPM-GAN is the Wasserstein GAN of \cite{AB17}. 
Here, the Wasserstein distance is used, corresponding to $h$ having Lipschitz constant at most $1$. 
...
Chapter \ref{chapter:latent-space-learning-theory} concerns estimation of $f$-divergences (and the KL-divergence in particular) between priors and aggregate posteriors, and so is directly relevant mutual information estimation via this equivalence.
...
\subsection{Wasserstein Autoencoders (WAEs)}
