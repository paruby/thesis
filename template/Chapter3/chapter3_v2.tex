...
For any distribution $\mathbb{P}_X$ on $\mathcal{X}$ we thus obtain the distribution of the variable $\tau(X)$ on $\mathcal{Y}$ as $\mathbb{P}_{\tau(X)} = \tau\left(\mathbb{P}_X\right)$ via the push-forward measure. \todo{Possibly change push-forward notation to use hash. Check compatibility with WAE section.}
...
% (cf.\ proofs in Appendix~\ref{first_properties:appendix}).
\medskip
...
\begin{proof}
    ...
    Since $\tau_{ZY}$ and $\tau_{YX}$ are exact it follows that for all $i\in\mathcal{I}_X$. \todo{Check this:}
    \begin{align*}
...
\subsection{Work building on this}
