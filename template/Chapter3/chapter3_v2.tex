...
In particular, this work seeks to understand the implications of modelling causal structure at a different level of \emph{abstraction} compared to the `truth'. For instance, though time invariably plays a role in any causal influence in the real world, mathematical models of causal structure may often omit any explicit reference to time. 
Though the work does not seek to propose new algorithms for causal inference, it has implications to existing algorithms by providing a framework for understanding the limitations of previous approaches.
