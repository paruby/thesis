...
Mathematically modelling these cases is interesting in part because their treatment requires consideration of issues that are not present in the acyclic, feedback-free case.
...
In the case of no latent confounders, once $\mathcal{G}$ has been identified, learning the functional relationships between parents and children reduces to regression. As such, we will only discuss methods for identifying $\mathcal{G}$.
...
Let $\mathcal{G}$ be a DAG with vertex set $V$ and edge set $\mathcal{E}$, and define a \emph{path} to be a sequence of consecutive edges of either directionality. 
...
The means that if $\mathbb{P}_X$ is Markov with respect to $\mathcal{G}$, it is also Markov with respect to $\mathcal{G}'$.
\medskip
...
However, the set graphs with respect to which $\mathbb{P}_X$ is Markov is `too large', since $\mathbb{P}_X$ is Markov with respect to any DAG $\mathcal{G}'$ whose edges contain those of $\mathcal{G}$.
...
For each pair of variables $u$ and $v$, one first searches for a set of other variables $Z_{uv}$ such that $u \perp v| Z_{ab}$. 
An undirected edge is drawn between $u$ and $v$ if no such $Z_{ab}$ can be found, since pair of nodes are adjacent if and only if there is a set that d-separates them \cite{elem causality, lemma 7.8}. 
The undirected arrows are then oriented as much as possible by identifying v-structures ($u \rightarrow z \leftarrow v$).
This is done by exploiting the fact that only out of all possible directed graphs consistent with the skeleton $u - z- v $\footnote{$u \rightarrow z \rightarrow v$, $u \leftarrow z \leftarrow v$, $u \leftarrow  z \rightarrow v$ and $u \rightarrow z \leftarrow v$}, it is only the v-structure in which $u$ and $v$ are \emph{not} d-separated by $z$.
The remaining edges are then oriented as much as possible such that any other orientation would lead to new v-structures or cycles. 
...
Note that in general, the search over sets of vertices $Z_{uv}$ for each pair $(u,v)$ can be in the worst case combinatorially expensive.
...
This is a hard problem which we will not discuss further here.
...
Under these conditions, Independent Component Analysis (ICA) algorithms can be applied to recover the both the matrix $\mathbf{I}-\mathbf{A}$ and the distribution over $\mathbf{e}$ from only observational data.
In brief, the algorithm works by identifying linear combinations of the components of $\mathbf{x}$ that maximise independence. 
LiNGAM then prunes the matrix of linear weights to force small entries to be 0, and then finds an ordering on the components by rearranging the matrix to be upper triangular. 
...
\cite{peters2013identifiability} prove identifiability under the different assumption that the noise variables are independent Gaussians with equal variance.
Under this assumption, all model parameters are identifiable from observational data. 
