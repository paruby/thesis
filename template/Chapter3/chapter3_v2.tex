...
However, in many other cases where aggregate or indirect measurements of a complex system form the basis of a macroscopic description of the system, e.g. in the cholesterol example, there is little theory to explain whether this is justified or how the micro- and macro-descriptions stand in relation to each other.
Occasionally this lack of theory leads to apparent contradictions.
...
In each of these cases, the fine-grained model is the `truth' while the coarse-grained model is a convenient abstraction.\footnote{\emph{`Truth'} is in quotation marks because the fine-grained model may itself be a coarsening of an even more detailed model.} 
...
The inconsistency arose because two different interventions in the mirco-level (raise HDL and raise LDL) have different effects yet map to the same intervention at the micro-level (raise TC).
...
This corresponds to the intuitive notion that interventions on distinct causal variables can be combined, and can be formalised by imposing a \emph{partial ordering} $\leq_X$ on the set of interventions modelled in which $\doop(X_i=x_i) \leq \doop(X_i=x_i, X_j=x_j)$ and $\doop(X_j=x_j) \leq \doop(X_i=x_i, X_j=x_j)$. 
