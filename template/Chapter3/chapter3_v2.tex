...
Formally, the problem can be stated thus: Given \iid~draws from a distribution $P_X$ induced by an SEM with causal graph $\mathcal{G}$, estimate $\mathcal{G}$.
...
By placing suitable restrictions on the functional forms of the structural equations, additional assumptions can lead to identifiability results - that is, unambiguous recovery of the SEM from the observational distribution, in contrast to recovery only up to an equivalence class. 
In the following, we will outline some methods for inferring SEMs and the assumptions they require for identifiability. 
...
If the functions $g_i$ of an additive noise model are linear, the model can be written as
%
\begin{align*}
X = AX + E,
\end{align*}
...
This is an instance of linear Independent Component Analysis (ICA), a problem class that will be discussed in detail in Chapter \ref{chapter:ica}.
...
\cite{peters2013identifiability} instead consider linear additive noise models under the assumption that all noise variables are independent Gaussians with equal (unknown) variance.
It is shown that all model parameters are identifiable from observational data in this case. 
...
The framework of exact transformations between SEMs provides a partial solution to this issue, as it has been argued that if a higher level SEM $\mathcal{M}_Y$ is an exact transformation of a SEM $\mathcal{M}_X$, they are causally consistent. 
...
This is problematic because most real physical systems exhibit feedback, and many of these do not satisfy this assumption.
...
A high level model need not be a perfectly accurate and faithful representation of lower-level model in order to be useful. 
Indeed, in all models are an approximation to some degree.
...
Since its publication, \cite{beckers2019approximate} have investigated precisely this question, exploring several subtleties. 
