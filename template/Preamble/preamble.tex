...
% New paragraph spacing and indendation
...
\RequirePackage[backend=biber, style=authortitle, citestyle=authoryear, sorting=nty, natbib=true, maxbibnames=99,maxcitenames=2, dashed=false, uniquelist=false, uniquename=false]{biblatex}
...
\addtocategory{ignore}{gresele2019incomplete}
...
\tikzstyle{icavar}=[circle,draw,minimum size=2.5em, inner sep=0em]
\tikzstyle{icavarobs}=[circle,draw,minimum size=2.5em, inner sep=0em, fill=gray!30!white]
...
% allow repeating corollarys
\makeatletter
\newtheorem*{rep@corollary}{\rep@title}
\newcommand{\newrepcorollary}[2]{%
\newenvironment{rep#1}[1]{%
 \def\rep@title{#2 \ref{##1}}%
 \begin{rep@corollary}}%
...
\newrepcorollary{corollary}{Corollary}
\newreplemma{proposition}{Proposition}
