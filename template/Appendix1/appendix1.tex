...
\begin{proof}
By Lemma~\ref{theorem:transitivity}, compositions of exact transformations are exact and so it suffices to prove this result for the marginalisation of a single childless variable.
...
\begin{proof}
By Lemma~\ref{theorem:transitivity}, compositions of exact transformations are exact and so it suffices to prove this result for the marginalisation of a single never-intervened-upon variable.
...
%
\begin{align*}
\mathcal{S}_X &= \left\lbrace W_i = E_i \:  : \: 1 \leq i \leq n  \right\rbrace  \cup \left\lbrace Z_i = \sum_{j=1}^n A_{ij}W_j  + F_{i} \:  : \: 1\leq i \leq m \right\rbrace, \\
%
\mathcal{I}_X &= \Big{\{} \nulli, \ \doop(W= w), \ \doop(Z= z), \ \doop(W= w, Z= z ) :   w \in \mathbb{R}^{n}, \, z \in \mathbb{R}^m \Big{\}},
%
\end{align*}
%
